export const nxVersion = '^12.0.0';

export const nsNxPluginVersion = '~2.0.0';
export const nsCoreVersion = '~8.0.0';
export const nsTypesVersion = '~8.0.0';
export const nsThemeVersion = '~1.0.4';
export const nsWebpackVersion = 'dev';
export const sassVersion = '^1.32.0';
export const nsIOSRuntimeVersion = '8.0.0';
export const nsAndroidRuntimeVersion = '8.0.0';


// Frontend frameworks
export const angularVersion = '~12.0.0';
export const nsAngularVersion = 'alpha';
export const nsNgToolsVersion = '~12.0.0';
export const rxjsVersion = '~6.6.0';
export const zonejsVersion = '^0.11.1';
export const typescriptVersion = '~4.2.0';
